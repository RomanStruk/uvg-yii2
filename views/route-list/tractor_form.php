...
$this->params['breadcrumbs'][] = ['label' => 'Трактори', 'url' => ['tractor']];
...
<div class="route-list-form">
    ...
    <?= $form->field($model, 'number')->textInput(['value'=>$model->getLastNumber('tractor')+1]) ?>
    <?= $form->field($model, 'driver_id')->dropDownList(\app\models\Drivers::getDriversList()) ?>
    <?= $form->field($model, 'transport_id')->dropDownList(\app\models\Transport::getList('tractor')) ?>
    <?= $form->field($model, 'type')->hiddenInput([ 'value'=>'tractor'])->label(false) ?>
    <?= $form->field($model, 'datego')->widget(DatePicker::className(), ['clientOptions' => ['autoclose' => true,'format' => 'yyyy-mm-dd']]) ?>
    <?= $form->field($model, 'fuel[default][filled_by]')->textInput()->label('Заправлено дизельно пального') ?>
    <?= $form->field($model, 'fuel[default][write_off]')->textInput()->label('Списано дизельно пального') ?>
    <?= $form->field($model, 'fuel[gasoline][filled_by]')->textInput(['value' => 0])->label('Заправлено бензину') ?>
    ...
    <?= $form->field($model, 'worked')->textInput(['value' => 8]) ?>
    <?= $form->field($model, 'kilometrage')->hiddenInput([ 'value'=>'0'])->label(false) ?>
